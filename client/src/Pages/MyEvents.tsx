...
async function reload(id:number){
    ...
    const events = allevents.map((i:any) => {
        ...
        let _id = i.id
        return {name:i.Name, description:i.Description, address:i.Address, time:_time, date:_date, members: null, id:_id}
    })
    return events;
}
...
function MyEvents(props: { id:any}) {
    ...
    const addEvent = async (
        ...
        // console.log(thedate)
        let _tmp = await API.newEvent(name, description, address, thedate, props.id)
        const e = events.slice();
        e.push({ name, address, time, date, description, id:_tmp.id, members: null });
        setEvents(e);
    };
