...
async function login(email:string, password:string):Promise<Message>{
        const cityRef = Fire.firestore().collection('Users').doc('moniera@umich.edu');
        ...
        .then((res:any)=>{ 
                ...
                .then((snapshot:any)=>{
                        if(!snapshot.exists){
                                ...
                                Fire.firestore().collection('Users').doc(email).set(user)
...
async function reset_password(_email:string):Promise<Message>{
        return Fire.auth().sendPasswordResetEmail(_email)
...
async function getall():Promise<Message>{
        return Fire.firestore().collection("Users").get()
...
async function changeemail(oldemail:any, newemail:string):Promise<Message|undefined>{
        ...
        .then( () =>{
                return Fire.firestore().collection("Users").doc(oldemail).update({email:newemail})
