...
async function login(email:string, password:string):Promise<Message>{
        const cityRef = Fire.firestore().collection("Users-WEB").doc("moniera@umich.edu");
...
async function new_user(email:string,  password:string, fName:string, lName:string):Promise<Message>{
        ...
        .then((res:any)=>{ 
                return Fire.firestore().collection("User-WEB").doc(email).get()
                .then((snapshot:any)=>{
                        if(!snapshot.exists){
...
async function getall():Promise<Message>{
        return Fire.firestore().collection("User-WEB").get()
